...
def get_kafka_consumer_group_id() -> int:
    ...
    group_id: str = os.getenv("CATS_CONSUMER_GROUP_ID", "default_group")
...
# Dictionary to store k counts
keyword_counts = defaultdict(int)

def process_message(message: str) -> None:
    """
    Process a single JSON message from Kafka and track k frequency.
    ...
    try:
        ...
        if isinstance(message_dict, dict):
            ...
            # List of Keywords
            keywords = ['cats', 'purr', 'zoomies', 'whiskers', 'meow']

            # Count occurrences of each k in the message
            for k in keywords:
                keyword_count = text.lower().count(k)
                if keyword_count > 0:
                    keyword_counts[k] += keyword_count
                    logger.info(f"Keyword '{k}' found {keyword_count} time(s).")

            # Log the updated keyword counts
